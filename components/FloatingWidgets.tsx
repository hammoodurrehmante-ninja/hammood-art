...
export default function FloatingWidgets() {
  ...
  useEffect(() => {
    fetch('/api/admin/settings')
      ...
      .then(d => { if (d?.whatsapp) setWa(d.whatsapp) })
      .catch(() => {})
  }, [])

  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' })

  const pos = wa.position || 'bottom-left'
      ...
      <div style={wrapStyle}>
        {wa.enabled && (
          ...
          >
            ...
            <span className="fw-tooltip">WhatsApp</span>
          </a>
        )}
      </div>

      <button
        className={`fw-btn fw-top${visible ? ' fw-top--visible' : ''}`}
        onClick={scrollTop}
        aria-label="Scroll to top"
        style={{ position: 'fixed', zIndex: 999, right: '24px', bottom: '24px' }}
      >
